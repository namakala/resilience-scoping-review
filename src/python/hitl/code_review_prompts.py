"""Interactive prompts for code review HITL CLI.

Provides ``_prompt_action``, ``_handle_merge_interactive``, and
``_show_more_context`` using ``questionary`` for user interaction.
"""
...
def _prompt_action(code_name: str) -> str:
...
def _handle_merge_interactive(con, source_code, db_path=None):
