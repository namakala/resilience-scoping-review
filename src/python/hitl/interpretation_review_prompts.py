"""Interactive prompts for interpretation review HITL CLI.

Provides ``_prompt_interpretation_action``, ``_prompt_edit_narrative``,
and ``_handle_split_interactive`` using ``questionary`` for user interaction.
"""
...
from .interpretation_review_queries import _get_interpretation_themes
...
def _prompt_edit_narrative(current_narrative: str) -> Optional[str]:
    """Prompt the user to edit the interpretation narrative.
